var Entry, History, Location, URL, html, jsdom, qs, util;
...
History = (function() {
  function History(browser) {
    ...
    resource = __bind(function(url, method, data, headers) {
      ...
      if (!(url.protocol === "file:" || (url.protocol && url.hostname))) {
        throw new Error("Cannot load resource: " + (URL.format(url)));
      }
      if (browser.window.document) {
        browser.open({
          history: this,
          interactive: browser.evaluate("window.top == this")
        });
      }
      options = {
        deferClose: false,
      ...
      return browser.window.resources.request(method, url, data, headers, __bind(function(error, response) {
        if (error) {
          ...
          html = response.body.trim() === "" ? "<html><body></body></html>" : response.body;
          document.write(html);
          document.close();
...
Location = (function() {
  function Location(history, url) {
    ...
    this.__defineSetter__("href", function(new_url) {
      return history._assign(URL.resolve(url, new_url));
    });
