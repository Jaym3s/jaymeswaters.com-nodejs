var Browser, EventLoop, History, core, html, jsdom;
...
Browser = (function() {
  ...
  function Browser(options) {
    var OPTIONS, Screen, TEXT_TYPES, cache, cookies, findOption, interact, k, mouseEventNames, resources, setCheckbox, storage, v, window, ws, xhr;
    ...
    this.on("error", __bind(function(err) {
      if (this.listeners("error").length === 1 || this.debug) {
    ...
    window = null;
    this.open = __bind(function(features) {
      var history, newWindow, _ref;
      if (features == null) {
        features = {};
      }
      if ((_ref = features.interactive) == null) {
      ...
      newWindow._evaluate = function(code, filename) {
        if (typeof code === "string" || code instanceof String) {
          return newWindow.run(code, filename);
        } else {
          return code.call(newWindow);
        }
      };
      ...
      newWindow.__defineGetter__("title", __bind(function() {
        var _ref2, _ref3;
        return (_ref2 = this.window) != null ? (_ref3 = _ref2.document) != null ? _ref3.title : void 0 : void 0;
      }, this));
      newWindow.__defineSetter__("title", __bind(function(title) {
        var _ref2, _ref3;
        return (_ref2 = this.window) != null ? (_ref3 = _ref2.document) != null ? _ref3.title = title : void 0 : void 0;
      }, this));
      newWindow.navigator.userAgent = this.userAgent;
      newWindow.navigator.javaEnabled = function() {
        return false;
      };
      ...
      newWindow.JSON = JSON;
      newWindow.Image = function(width, height) {
        var img;
    ...
    this.fire = function(name, target, options, callback) {
      var event, key, type, value, _ref, _ref2;
      ...
      type = options.type || (__indexOf.call(mouseEventNames, name) >= 0 ? "MouseEvents" : "HTMLEvents");
      event = window.document.createEvent(type);
      event.initEvent(name, !!options.bubbles, !!options.cancelable);
      if (options.attributes != null) {
        _ref2 = options.attributes;
        for (key in _ref2) {
          value = _ref2[key];
          event[key] = value;
        }
    ...
    setCheckbox = __bind(function(selector, value, callback) {
      var field;
      field = this.field(selector);
      if (field && field.tagName === "INPUT" && field.type === "checkbox") {
        ...
        if (field.checked ^ value) {
          this.fire("click", field, callback);
        } else if (callback) {
          callback(null, false);
        }
        return this;
    ...
    this.check = function(selector, callback) {
      return setCheckbox(selector, true, callback);
    };
    this.uncheck = function(selector, callback) {
      return setCheckbox(selector, false, callback);
    };
    this.choose = function(selector, callback) {
    ...
    this.attach = function(selector, filename, callback) {
      var field;
      field = this.field(selector);
    ...
    this.select = function(selector, value, callback) {
      var option;
      option = findOption(selector, value);
      this.selectOption(option, callback);
      return this;
    };
    this.selectOption = function(option, callback) {
      var select;
    ...
    this.unselect = function(selector, value, callback) {
      var option;
      option = findOption(selector, value);
      this.unselectOption(option, callback);
      return this;
    };
    this.unselectOption = function(option, callback) {
      var select;
      if (option && option.getAttribute("selected")) {
        select = this.xpath("./ancestor::select", option).value[0];
        if (!select.multiple) {
